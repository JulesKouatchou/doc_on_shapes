...
!# Test Shapes
!
! This program is meant to test the OOP features 
! (namely abstraction, encapsulation and inheritance) in Fortran.
! 
! We define various geometric shapes and write functions
! to compute their area and perimeter.
!
program test_shapes
   ...
   rtri =  triangle( a=3.0, b=4.0, c=5.0, name=name_ptr)
   ...
   write(6,'(1x,a)'  )'---------------------------------------------'
   do i = 1,size(list)
      write(fl, '(i0)') len(trim(list(i)%shap%get_name()))
      il = 24 - len(trim(list(i)%shap%get_name()))
      write(sl, '(i2)') il
      write(fmt,"(a)") '(1x,a'//fl//','//sl//'x)'
      write(6,fmt, advance='no') adjustl(list(i)%shap%get_name())
      write(6,'(2(f8.4,4x))') list(i)%shap%get_area(), list(i)%shap%get_perimeter()
   end do
